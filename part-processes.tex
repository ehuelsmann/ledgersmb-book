...
There's only one way to persist an invoice in LedgerSMB and that is by posting it. This means
...
\item Accounting principles demand an audit trail of transactions
\end{enumerate}
...
using the same invoice number; how is that \gls{customer} supposed to verify and document
...
Depending on whether the customer or vendor has already paid the invoice there are several
options to handle this situation:

\begin{description}
\item [Credit Invoice] The \gls{customer} has not paid the invoice yet
\item [Credit Note] The customer has paid the invoice and allows offsetting against
   other (possibly future) invoices
\item [Vendor Invoice] The \gls{vendor} has not paid the invoice yet
\item [Debit Note] The vendor has paid the invoice and allows offsetting against
other (possibly future) invoices
\end{description}
...
\section{Receipts and payments in foreign currencies\index{foreign currency}}
...
LedgerSMB assumes:
\begin{itemize}
\item A company has one functional base currency\index{functional base currency}.
\item Customers and vendors may transact in different currencies.
\item The foreign currency amount remains associated with the document so open invoices can be settled in their original currency.
\item Each AR or AP document records:
\begin{itemize}
\item the transaction currency (EUR, GBP, CAD, etc.)
\item the exchange rate used
\item base-currency values posted to the GL
\end{itemize}
\end{itemize}

\subsection{Exchange rates\index{Exchange rates}}
...
\item multiple exchange-rate classes
\item validity periods for rates
\item multiple rates on the same day (introduced in 1.7)
\end{itemize}
...
\subsection{Exchange rate classes\index{Exchange rate classes}}
...
\begin{itemize}
\item operational transaction entry
\item revaluation
\item translation
\item other accounting requirements
\end{itemize}
...
For AR and AP, LedgerSMB's foreign currency workflow\index{foreign currency workflow} is:
\begin{enumerate}
\item Enter the invoice in the customer's or vendor's currency.
\item Record the exchange rate for that specific transaction.
\item Post the GL in the company's functional currency.
\item Retain the original foreign amount on the open document.
\item When payment is recorded, compare the original and settlement exchange rates.
\item Automatically recognize any realized foreign exchange gain or loss.
\item Support different exchange rates for different transactions on the same day, reflecting actual bank or negotiated rates.
\end{enumerate}
