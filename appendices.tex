...
The following examples use the  \texttt{lsmb\_dbadmin} for the database administrative user. 
Recent LedgerSMB installs do not automatically create an  \texttt{lsmb\_dbadmin}\index{lsmb\_dbadmin} user.
This has to be created manually.  You may have to use the \texttt{postgres} user depending on how your database was installed.
...
\begin{enumerate}
    ...
    \item Create a new database. 
        ...
        Do not create the database using \texttt{setup.pl} as the restore in the next step will setup the company.   The \texttt{psql} syntax is:
    ...
    \item Verify everything restored correctly.
