...
\item The web server (as an application - \gls{cgi})
...
move is to reduce the \gls{cgi} layer to become more of a ``glue'' layer between
...
One of the roles that has remained so far for the \gls{cgi} layer is to generate
...
\item Perl 5.32 or higher, with additional modules
\item PostgreSQL 10.0 or higher, with contribs
