...
details. \secref{sec-account-options} discusses the options in detail - for now using the
...
Based on his reading from \secref{sec-definition-parts}, Jack decides to enter the hard
...
look up the parts efficiently later on. Returning his reading to \secref{sec-definition-parts},
...
Using the work flow detailed in \secref{sec:creating-customers-and-vendors} Jack starts to do so by going through the menu \menupath{AP \ma Vendors \ma Add Vendor}.
...
For more detail, the reader is referred to \secref{subsec:sending-quotation-by-email}.
