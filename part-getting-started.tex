...
These control the formatting of reports like invoices, balance sheet, income statement, etc. 
...
Jack selects  \texttt{demo} to load the demo templates.
...
The setup.pl program allows the user to be created with the following permissions:

\begin{itemize}
\item Manage Users
\item Full Permissions
\item No changes
\end{itemize}

Jack's choice for a fully authorized \texttt{admin} user leads him to select the
\texttt{Full Permissions} option.
...
one of the default  chart of accounts was used in as shown in \charef{cha-company-creation} or 
...
\url{https://example.com/ledgersmb/login.pl} to access the log in screen.
...
After entering all of the information and taping the \texttt{Login} button Jack will see an alert letting
...
\begin{samepage}
Jack enters his new information as follows, then clicks the \texttt{Change Password} button:
\begin{description}
...
Setting system defaults is optional.  Out of the box, LedgerSMB contains reasonable system defaults.
Jack navigates to System $\rightarrow$ Defaults and sees the screen depicted in \figref{fig:first-user-system-defaults}.
