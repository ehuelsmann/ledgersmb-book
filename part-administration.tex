...
Tax forms\index{tax forms} are meant to support tax reporting requirements in the countries
where the company has tax or other government reporting obligations. This means that tax forms
are country specific: each country for which reporting obligations exist
needs to have its own form associated.

Using tax forms requires that:
\begin{itemize}
\item A template has to exists in order to format the tax report.  Two United States based templates are provided by default: 1099-MISC\index{1099-MISC} and 1099-INT\index{1099-INT}, provided they were loaded during the company setup. To view them navigate to \menupath{System \ma Templates} and select them in the ``Template'' drop down. For more information on templates see \secref{subsec-company-config-templates}
\item A tax form has to be created for each type of tax reporting or jurisdiction. See \secref{subsec-tax-taxforms-creation}.
\item  Each customer and vendor that has government reporting requirements needs to be
linked to a tax form. \secref{subsec-business-processes-customers-creating-account} describes how to do this.
\end{itemize}
...
Tax forms have the following fields:
\begin{description}
...
\item [Select by Default] Determines if the tax form check mark on invoices is checked or unchecked by default when the form is loaded
\item [Accrual Basis] Use accrual based tax reporting for the report instead of cash based.
\end{description}

\subsection{Using tax forms}
\label{subsec-tax-taxforms-using}

To print the Tax Form navigate to \menupath{Tax Forms \ma Reports} where there are selections for which tax form, customer number, vendor number, date range etc.
