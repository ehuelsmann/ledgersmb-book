...
\label{subsubsec-parts-taxation}
...
\item[Payment (AR\_paid)] This check mark adds the account to the list of accounts
   ...
   adds the account to the part entry screen as described in \secref{sec-definition-parts}.
...
When an account has been marked as a Tax account (see \secref{sec-account-options}, item
...
and part settings and service settings described in \secref{subsec-parts-taxation} and
\secref{subsec-service-taxation} respectively.
\end{quotation}
...
linked to it. \secref{subsec:customers-creating-account} describes how to do this.
...
(or exclude) invoice lines from the tax form. See \secref{subsec:creating-invoices}
...
are covered in \appref{cha:Migration}. The same is true for upgrades from SQL Ledger 2.6 or 2.8.
