...
\item [Default accounts] This setting will be used to pre-select an account in
...
\item Expense
\end{itemize}
\item [Foreign exchange gain and loss accounts] When working with foreign currencies,
...
\item [Default country] This setting indicates which country needs to be pre-selected
   in country selection lists.
\item [Default language] The language to be used when no other language has been selected. Several parts of the
application require language selection, such as customer, vendor and employee entry screens.

\item [Templates directory] This setting indicates which set of templates - stored in the
   ...
   \item xedemo which contains the same templates based on TeTeX, which handles UTF-8 input (and thus accented characters) much better than \LaTeX and is broadly available, but not usually preinstalled.
   \end{itemize}
\item [List of currencies \& default currency] Enter a list of all currencies you want
...
\item [Company data (name /address)] The fields ``Company Name'', ``Company Address'',
``Company Phone'' and ``Company Fax'' will be used on printed/e-mailed invoices.
\item [Password duration] This field indicates the duration passwords will be considered valid. Freshly set passwords will expire after this period.\footnote{If this value isn't set, passwords will expire after 1 year by default.} Note that passwords for freshly
created users expire within 24 hours after creation. @@@ What's the input here? A number? Would that number be days, weeks, years?
\item [Default Email addresses] These addresses will be used to send e-mails from the system.
...
``$\langle$'' and ``$\rangle$''.
\item [Max per dropdown] Some elements in the screens may present a drop down. However, drop downs are
...
\item [Item numbering] Many items in the system have sequence numbers: invoices, transactions, parts, etc. These sequence numbers can be just a number (i.e. 1 or 37),
...
\item [Check prefix] The prefix to use when printing checks. There's no check sequence number. That sequence number is requested from the check printing interface, because
...
\subsection{Standard Industry Code}
