...
\item Special purpose accounts
\item Default country
\item Default language
\item Template set to be used
\item List of currencies / default currency
\item Company data (name /address)
\item Password validity duration
\item E-mail address configuration
\item Max items per dropdown
...
\section{System Defaults screen settings}
...
can themselves be assigned to other headers resulting in trees of account groups.
...
can be maintained through the System -> Chart of accounts -> Add GIFI and List GIFI menu items.
