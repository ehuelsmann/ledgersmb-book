...
There are a number of ways to divide customer, vendor or entity data into smaller pieces for reporting and analysis. \lsmbsp calls these divisions \gls{reporting units}.

Reporting units\index{reporting units} can be created (added), modified or deleted through the \menupath{System \ma Reporting Units} menu item.

The default install includes department\index{department reporting units}, project\index{project reporting units}, job\index{job reporting units}, fund\index{fund reporting units}, customer\index{customer reporting units}, vendor\index{vendor reporting units}, and lot\index{lot reporting units}.
However, these are all marked inactive in the default install. 

Each reporting unit can be assigned to show on Sales, Purchases, GL, Entity, Manufacturing, Fixed Assets, or Time Card views.

Once the overall reporting unit is defined using the ``Reporting Unit'' view, it has to be assigned to a specific entity.
This is done by clicking on the ``Add Unit'' button, which defines and makes the specific department, job, etc. available to \lsmb.
