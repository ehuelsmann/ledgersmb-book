...
For the default, non debug \texttt{ledgersmb.yaml} \index{ledgersmb.yaml} see  \url{https://github.com/ledgersmb/LedgerSMB/blob/master/doc/conf/ledgersmb.yaml}
...
Email \index{mail} can be configured by selecting which of the two available transports to use. The default \texttt{ledgersmb.yaml} file contains examples of both.

\begin{description}
\item{\texttt{Email::Sender::Transport::Sendmail}} – The email is sent using the local server's \texttt{sendmail} \index{sendmail} binary. The options are:
    \begin{description}
        \item{\texttt{transport:\$class}} – The required transport to use.
        \item{\texttt{transport:path}} – optionally provide a path to the directory that contains the 'sendmail' binary.
    \end{description}
    
\item{ LedgerSMB::Mailer::TransportSMTP} - This email \index{email} is sent using a remove SMTP server.  The options are:
    \begin{description}
        \item{\texttt{transport:\$class}} – The required transport to use.
        ...
        \item {\texttt{sasl\_username:\$class}} – The required smtp server authentication method
        \item {\texttt{sasl\_username:mechanism}} – @@@TODO What options are available here?
        \item {\texttt{sasl\_username:callback:user}} – The required SMTP user name.  @@@TODO I assume 'the-user' is just a place holder.
        \item {\texttt{sasl\_username:callback:pass}} – The required SMTP password.  @@@TODO what does 'SECURITY-FIRST' mean?  Is this just a place holder or does it have meaning to pick up the password from some other place?
    \end{description}
...
\begin{description}
    ...
    \item {\texttt{config:sql}} – @@@TODO
    \item {\texttt{config:sql\_data}} – @@@TODO
    \item {\texttt{config:templates}} – Templates base directory. Typically set to \texttt{templates}.
    \item {\texttt{config:UI}} – @@@TODO
    \item {\texttt{config:UI\_cache}} – @@@TODO
