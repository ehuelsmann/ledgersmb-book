...
For example, "Project 1" could be the parent of "Assessment", "Planning", "Execution", and "Finalize".  This allows tracking of project phases or sub-components.

Reporting unit classes can be created (added), modified or deleted through the \menupath{System \ma Reporting Units} menu item. Reporting units can be created and modified using the same menu item.  Reporting units have start and end dates, but may not be deleted to ensure financial integrity.
...
However, these are all marked inactive in the default install and they have no defined reporting units. The result is that reporting unit class selections do not show anyplace in the user interface by default.
...
\begin{longtable}{ llp{6cm} }
	...
	AR & Shows in AR Transaction, Sales Invoice, Credit Note,  \\
         & Credit Invoice, Customer Return, Aging Report screens\\
	AP & Shows in AP Transaction, Vendor Invoice, Debit Note, \\
	 &  Debit Invoice, Aging Report screens\\
	GL &  Shows in General Ledger Transaction, Trial Balance, \\
	 & Add budget, Income Statement screens\\
	Entity &  Not currently available\\
...
\begin{longtable}{ llp{6cm} }
	Field  & Description \\ \hline
	\endhead
	Control Code & The data entry short code, must be unique for all reporting units\\
	...
	Customer &  Not used, will be removed \\
	\\
